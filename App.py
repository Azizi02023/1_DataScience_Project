...
# Display header
st.header('Car Price Prediction Using a Machine Learning Model')

df = pd.read_csv('Cardetails.csv')
def get_brand_nameofCar(Car_name):
    Car_name = Car_name.split(' ')[0]
    return Car_name.strip(' ')

df['name'] = df['name'].apply(get_brand_nameofCar)
name = st.selectbox('Select Car Bran', df['name'].unique())
...
engine = st.slider('Engine CC',700,500)
